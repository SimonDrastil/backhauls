  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-4xl flex-col gap-12 px-6 py-16">
      <section className="text-center">
        <p className="text-xs uppercase tracking-[0.2em] text-brand/70">Subscription tiers</p>
      ...
      <section className="grid gap-6 md:grid-cols-3">
        {plans.map((plan) => (
          <div key={plan.name} className="rounded-3xl border border-brand/10 bg-white p-6 shadow">
            ...
            <Link
              href="/"
              className="mt-6 inline-flex w-full items-center justify-center rounded-full bg-brand px-4 py-2 text-sm font-semibold text-white transition hover:bg-brand-dark"
            >
              Choose plan
            </Link>
          </div>
